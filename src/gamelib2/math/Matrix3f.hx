...
class Matrix3f
{
  ...
  public function get(i : Int, j : Int) : Float
  {
    ...
    trace("Fatal: Invalid matrix index.");
    throw ("Invalid matrix index. " + i + ", " + j);
  }
   ...
   */
  public function getAsArray(data : Array<Float>, rowMajor : Bool) : Void
  {
    if (data.length == 9) 
    {
      if (rowMajor) 
    ...
    {
      throw ("Array size must be 9 or 16 in Matrix3f.get().");
    }
  }
   ...
   */
  public function getColumn(i : Int) : Vector3f
  {
    return getColumnStore(i, null);
  }
   ...
   * @return the column specified by the index.
   */
  public function getColumnStore(i : Int, store : Vector3f) 
  {
   ...
   */
  public function getRow(i : Int) : Vector3f
  {
    return getRowStore(i, null);
  }
   ...
   * @return the row specified by the index.
   */
  public function getRowStore(i : Int, store : Vector3f) : Vector3f
  {
    if (store == null) store = new Vector3f();
    ...
    switch (i) 
    {
      ...
      default:
      {
        trace("Fatal: Invalid row index. " + i);
        throw ("Invalid row index. " + i);
      }
    }
  ...
  */
  ...
  public function setColumn(i : Int, column : Vector3f) : Void
  {
    ...
    switch (i) 
    {
      ...
      default:
      {
        trace("Fatal: Invalid column index. " + i);
        throw ("Invalid column index. " + i);
      }
    }
  }
  ...
  public function setRow(i : Int, row : Vector3f) : Void
  {
    if (row == null) 
    {
      trace("Warning: Row is null. Ignoring.");
      return;
    }
    ...
    switch (i) 
    {
      ...
      default:
      {
        trace("Warning: Invalid row index. " + i);
        throw ("Invalid row index. " + i);
      }
    }
  }
  ...
  public function set(i : Int, j : Int, value : Float) : Void
  {
    ...
    trace("Fatal: Invalid matrix indices. " + i + ", " + j);
    throw ("Invalid matrix indices. " + i + ", " + j);
  }
  ...
  */
   ...
   */
  public function setFromArray(matrix : Array<Float>, ?rowMajor : Bool = true) : Void
  {
    if (matrix.length != 9) throw ("Array must be of size 9.");
  ...
  {
    quaternion.toRotationMatrix3Store(this);
  }
  ...
  /**
   * <code>fromAngleAxis</code> sets this matrix3f to the values specified
  ...
  /**
   * <code>fromAngleNormalAxis</code> sets this matrix3f to the values
  ...
  public function fromAngleNormalAxisEx(cosA : Float, sinA : Float, axis : Vector3f) : Void
  {
      var fCos = cosA;
      var fSin = sinA;
      var fOneMinusCos = 1.0 - fCos;
   ...
   */
  public function mult(mat : Matrix3f) : Matrix3f
  {
    return multMatrix3Store(mat, null);
  }
   ...
   */
  public function multMatrix3Store(mat : Matrix3f, product : Matrix3f) : Matrix3f
  {        
    if (product == null) product = new Matrix3f();

    var temp00 = m00 * mat.m00 + m01 * mat.m10 + m02 * mat.m20;
    var temp01 = m00 * mat.m01 + m01 * mat.m11 + m02 * mat.m21;
    var temp02 = m00 * mat.m02 + m01 * mat.m12 + m02 * mat.m22;
    var temp10 = m10 * mat.m00 + m11 * mat.m10 + m12 * mat.m20;
    var temp11 = m10 * mat.m01 + m11 * mat.m11 + m12 * mat.m21;
    var temp12 = m10 * mat.m02 + m11 * mat.m12 + m12 * mat.m22;
    var temp20 = m20 * mat.m00 + m21 * mat.m10 + m22 * mat.m20;
    var temp21 = m20 * mat.m01 + m21 * mat.m11 + m22 * mat.m21;
    var temp22 = m20 * mat.m02 + m21 * mat.m12 + m22 * mat.m22;
   ...
   */
  public function multVector3(vec : Vector3f) : Vector3f
  {
    return multVector3Store(vec, null);
  }
   ...
   */
  public function multVector3Store(vec : Vector3f, product : Vector3f) : Vector3f
  {        
    if (null == product) 
    {
   ...
   */
  public function multScalarLocal(scale : Float) : Matrix3f
  {
    m00 *= scale;
    m01 *= scale;
    m02 *= scale;
   ...
   */
  public function multVector3Store2(vec : Vector3f) : Vector3f
  {
   ...
   */
  public function multLocal(mat : Matrix3f) : Matrix3f
  {        
    return multMatrix3Store(mat, this);
  }
  ...
  public function transposeLocal() : Matrix3f
  {
    ...
    getAsArray(tmp, false);
    setFromArray(tmp, true);
   ...
   */
  public function invert() : Matrix3f
  {
   ...
   * @return The store
   */
  public function invertStore(store : Matrix3f) : Matrix3f
  {
  ...
  public function invertLocal() : Matrix3f
  {
    ...
    if ( Math.abs(det) <= 1.1920928955078125E-7/*Math.FLT_EPSILON*/ )
    {
      return zero();
    }

    var f00 = m11*m22 - m12*m21;
    var f01 = m02*m21 - m01*m22;
    var f02 = m01*m12 - m02*m11;
    var f10 = m12*m20 - m10*m22;
    var f11 = m00*m22 - m02*m20;
    var f12 = m02*m10 - m00*m12;
    var f20 = m10*m21 - m11*m20;
    var f21 = m01*m20 - m00*m21;
    var f22 = m00*m11 - m01*m10;
   ...
   */
  public function adjoint() : Matrix3f
  {
   ...
   * @return store
   */
  public function adjointStore(store : Matrix3f) : Matrix3f
  {
    if (store == null) 
    {
      store = new Matrix3f();
  ...
  {
    var fCo00 = m11*m22 - m12*m21;
    var fCo10 = m12*m20 - m10*m22;
    var fCo20 = m10*m21 - m11*m20;
    var fDet = m00*fCo00 + m01*fCo10 + m02*fCo20;
    return fDet;
  }
  ...
  */
   ...
   */
  public function equals(m : Matrix3f) : Bool
  {
    return 
      (m00 == m.m00) && (m01 == m.m01) && (m02 == m.m02) &&
      (m10 == m.m10) && (m11 == m.m11) && (m02 == m.m12) &&
      (m20 == m.m20) && (m21 == m.m21) && (m02 == m.m22);
  }
  ...
  public function fromStartEndVectors(start : Vector3f, end : Vector3f) : Void
  {
    ...
    if (f > 1.0 - 0.0001) 
    {
      ...
      for (i in 0...3) 
      {
        for (j in 0...3) 
        {
          var val : Float = -c1 * u.get(i) * u.get(j) - c2 * v.get(i)
    ...
    else 
    {
      ...
      //var hvx, hvz, hvxy, hvxz, hvyz : Float;
      h = 1.0 / (1.0 + e);
      var hvx = h * v.x;
      var hvz = h * v.z;
      var hvxy = hvx * v.y;
  ...
  public static function equalIdentity(mat : Matrix3f) : Bool
  {
  ...
  public function multVector2(vec : Vector2f, ?store : Vector2f = null) : Vector2f
  {        
    if (null == store) 
    {
      store = new Vector2f();
    ...
    store.x = m00 * x + m01 * y + m02;
    store.y = m10 * x + m11 * y + m12;

    return store;
  }

  public function setTranslationComp(tx : Float, ty : Float)
  {
    m02 = tx;
    m12 = ty;
  }

  public function setTranslationVector(vec : Vector2f)
  {
    m02 = vec.x;
    m12 = vec.y;
  }

  public function setScale(vec : Vector2f)
  {
    m00 = vec.x;
    m11 = vec.y;
  }

  public function setScaleComp(sx : Float, sy : Float)
  {
    m00 = sx;
    m11 = sy;
  }
